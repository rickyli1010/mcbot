...
1. **Discord Bot (`discord-bot/`)**: A Node.js application that listens to Discord slash commands (`/ec2 start`, `/ec2 stop`, `/ec2 status`) to allow users to start, stop, and check the status of the EC2 instance hosting the Minecraft server. It uses `@aws-sdk/client-ec2` to interact with AWS.
...
1.  **Start:** In your Discord server, type `/ec2 action:start`. Wait a minute for the server to power on.
2.  **Status:** Type `/ec2 action:status` to get the public IP address.
3.  **Play:** Connect to your Minecraft server and play.
4.  **Stop:** If you want to manually stop the server, type `/ec2 action:stop`.
